 ...
 *   disagreed (`conflict`).
 */
export function resolveOrphanStrategy(
  rules: RemoveRule[],
  globalDefault: OrphanStrategy | undefined
): { strategy: OrphanStrategy; conflict: boolean } {
  const fallback: OrphanStrategy = globalDefault ?? 'remove-children';

  if (rules.length === 0) {
    return { strategy: fallback, conflict: false };
  }

  const strategies = new Set<OrphanStrategy>();
  for (const rule of rules) {
    strategies.add(rule.orphanStrategy ?? fallback);
  }

  const conflict =
    strategies.has('remove-children') && strategies.has('keep-shell');

  // Most destructive wins.
  const strategy: OrphanStrategy = strategies.has('remove-children')
    ? 'remove-children'
    : 'keep-shell';
