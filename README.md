...
- **Trace `.zip` files**: every `*.trace` entry (`test.trace`, `0-trace.trace`, ...) is processed as NDJSON — one JSON event per line, with steps stored as paired `"before"` / `"after"` events linked by `callId`/`parentId`. `*.network` entries are handled the same way, and `resources/` files are redacted. Removal rules match against the `"before"` event titles (`title`, or `apiName` on older versions); descendants are found by walking the full `parentId` tree, including nested `test.step` and `pw:api` children. Archives with a legacy `trace.json` are still supported as a fallback.
